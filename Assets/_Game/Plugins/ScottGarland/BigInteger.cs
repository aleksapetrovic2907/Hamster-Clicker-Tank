...
namespace ScottGarland.Numbers
{
    ...
    public class BigInteger
    {
        ...
        public static BigInteger Pow(int b, int power)
        {
        ...
        public static string[] notations = new string[] { "", "K", "M", "B", "T", "Q", "q", "S", "s", "O", "N", "D", "U", "DD" };

        public static string ToStringWithoutDecimals(BigInteger num)
        {
            string numString = num.ToString();
            int length = ((numString.Length - 1) % 3) + 1;
            int notationIndex = ((numString.Length - 1) / 3);

            return numString.Substring(0, length) + notations[notationIndex];
        }

        public static string ToStringWithDecimals(BigInteger num)
        {
            string numString = num.ToString();
            if (numString.Length < 4) return ToStringWithoutDecimals(num);

            int length = ((numString.Length - 1) % 3) + 1;
            int notationIndex = ((numString.Length - 1) / 3);

            return numString.Substring(0, length) + "." + numString.Substring(length, 1) + notations[notationIndex];
        }
    }
}